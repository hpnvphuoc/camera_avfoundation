...
void SetUpFCPCameraApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FCPCameraApi> *api, NSString *messageChannelSuffix) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.camera_avfoundation.CameraApi.getCameraLenPosition", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FCPGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(getCameraLenPositionWithCompletion:)], @"FCPCameraApi api (%@) doesn't respond to @selector(getCameraLenPositionWithCompletion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        [api getCameraLenPositionWithCompletion:^(NSNumber *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
